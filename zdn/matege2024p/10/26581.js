(function() {
	...
	retryWhileError(function() {
		...
		let key = "26581";
		let preference = ['speed_on_way_to_point_A', 'speed_on_way_to_point_B'];
		...
		let the_vehicleRacingOnRoad = sklonlxkand(["автомобиль", "мотоцикл", "велосипед", "электросамокат", "гироскутер",
			"мотоциклист", "велосипедист", "гонщик", "грузовик", "автомобилист"
		].iz());
		...
		NAtask.setTask({
			text: '' + the_vehicleRacingOnRoad.ie.toZagl() + ' ' + ['выехал', 'отправился', 'начал движение'].iz() +
				' с постоянной скоростью из ' + the_humanSettlementDestination.re + ' A в ' +
				the_humanSettlementDestination.ve + ' B, расстояние между которыми ' + ['равно ' + S + ' км', '(в км) равно ' +
					S
				].iz() + '. ' + the_afterAWhile.toZagl() +
				' он отправился обратно со скоростью на ' + a + ' км/ч ' + ['больше', 'быстрее'].iz() + [' прежней',
					', чем туда'
				].iz() + '. ' +
				'По ' + ['дороге', 'пути'].iz() + ' он ' + ['сделал остановку', 'остановился'].iz() + ['', ' у магазина',
					' в кафе'
				].iz() + ' на ' + [chislitlx(n, 'час'), (n * 60).toChMin()].iz() +
				'. В результате он затратил на ' + ['обратный путь', 'путь назад', 'путь обратно'].iz() +
				' столько же времени, ' +
				'сколько на путь ' + ['из A в B', 'туда'].iz() + '. ',
			questions: [[{
				text: the_orderToFind.toZagl() + ' скорость ' + the_vehicleRacingOnRoad.re + ' на пути ' + ['из A в B',
					'туда'
				].iz() + '.',
				answers: x,
			}, {
				text: the_orderToFind.toZagl() + ' скорость ' + the_vehicleRacingOnRoad.re + [' на пути ' + ['из B в A',
					'обратно', 'назад'
				].iz(), ' обратном пути'].iz() + '.',
				answers: x + a,
			}, ][rand]],
