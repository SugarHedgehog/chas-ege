(function() {
	...
	let key = "16";
	let preference = ['scope_of_all_work', 'work_on_first_day', 'work_on_second_day', 'number_of_days_to_work'];
	...
	*/
	...
	chas2.task.setCountableTask(
		[{
			...
			utv: 'ежедневно <replace1/> ' + action[1] + ' на одно и то же количество ' + product.rm +
				' больше по сравнению с предыдущим днём'
		}, {
			utv: 'за ' + knownDay1 + '-й день <replace1/> ' + action[2] + ' ' + chislitM(quantity1, product.ve, product.vm,
				product.rm),
			nah: rand == 1,
			zna: quantity1,
			vpr: 'сколько ' + product.rm + ' было ' + action[3] + ' за ' + knownDay1 + '-й день',
		}, {
			utv: 'за ' + knownDay2 + '-й день <replace1/> ' + action[2] + ' ' + chislitM(quantity2, product.ve, product.vm,
				product.rm),
			nah: rand == 2,
			zna: quantity2,
