...
CanvasRenderingContext2D.prototype.drawGridWithArrows = function(options = {}) {
	const {
			...
			stepByCeilX = 1,
			stepByCeilY = 1,
			typeX = 'standard', // time, custom
			typeY = 'standard', // time, custom
			labelsX = null,      // Массив подписей для оси X
			labelsY = null,      // Массив подписей для оси Y
	...
	// Ось X: обработка разных типов подписей
	switch (typeX) {
		case 'custom':
			if (labelsX.length != 0) {
				// Пользовательские подписи из массива
				for (let i = 0; i < labelsX.length; i++) {
					const xPos = i * cellWidth * stepByCeilX;
					this.fillText(labelsX[i].ts(), xPos, textOffsetY);
					this.drawLine(xPos, tickSize, xPos, -tickSize);
				}
			}
			break;
		...
		case 'time':
			...
			for (let i = 0; i < gridWidth && step1 <= maxX; i += cellWidth * stepByCeilX) {
				const label = `${cycleTime ? step1 % 24 : step1}:00`;
	...
	// Ось Y: обработка разных типов подписей
	switch (typeY) {
		case 'custom':
			if (labelsY.length != 0) {
				// Пользовательские подписи из массива
				for (let i = 0; i < Math.min(labelsY.length, Math.floor(gridHeight / (cellHeight * stepByCeilY))); i++) {
					const yPos = -i * cellHeight * stepByCeilY;
					this.fillText(labelsY[i].ts(), -textOffsetX, yPos);
					this.drawLine(-tickSize, yPos, tickSize, yPos);
				}
			}
			break;
		...
		case 'time':
			...
			for (let i = 0; i > -gridHeight && step2 <= maxY; i -= cellHeight * stepByCeilY) {
				const label = `${cycleTime ? step2 % 24 : step2}:00`;
...
CanvasRenderingContext2D.prototype.fillFunctionArea = function({ func, leftEnd, rightEnd, fillColor = om.transparentBrandColors.iz(), step = 0.01, bottomBound = 0 }) {
    ...
    // Перемещение к начальной точке (leftEnd, bottomBound)
    this.moveTo(leftEnd, -bottomBound);

    // Рисование кривой
    for (let x = leftEnd; x <= rightEnd; x += step) {
        const y = func(x);
        this.lineTo(x, y); // Инвертируем y для корректного отображения
    }

    // Замыкание пути до нижней грани
    this.lineTo(rightEnd, -bottomBound);
    this.lineTo(leftEnd, -bottomBound);
...
CanvasRenderingContext2D.prototype.drawMarkWithLabel = function({ scale = 20, point, length = 5, label , stepX = 3, stepY = 15, type = 'horizontal'}) {
    const [x, y] = point;
	...
	switch(type){
		case 'horizontal':
			this.drawLine(scale * x, scale * y + length, scale * x, scale * y - length);
			break;
		case 'vertical':
			this.drawLine(scale * x + length, scale * y, scale * x - length, scale * y);
	}
    
    if (label) {
        this.fillText(label, scale * x + stepX, scale* y + stepY);
    }
